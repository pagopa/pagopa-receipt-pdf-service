...
public class AttachmentResource {
  ...
  @Operation(
      ...
      description =
          "Retrieve the attachment linked to the provided third party data id from the provided"
              + " attachment url")
  @APIResponses(
      value = {
        @APIResponse(ref = "#/components/responses/InternalServerError"),
  ...
  public RestResponse<byte[]> getAttachment(
    ...
    logger.info(
        "Received get attachment with name {} for receipt with id {}", attachmentUrl, thirdPartyId);
    if (requestFiscalCode == null || requestFiscalCode.length() != FISCAL_CODE_LENGTH) {
      String errMsg = "Fiscal code header is null or not valid";
      throw new InvalidFiscalCodeHeaderException(PDFS_901, errMsg);
    }
    ...
    try (BufferedInputStream inputStream =
        new BufferedInputStream(new FileInputStream(attachment))) {
