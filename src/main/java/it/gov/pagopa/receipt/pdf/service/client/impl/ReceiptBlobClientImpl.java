...
public class ReceiptBlobClientImpl implements ReceiptBlobClient {
   ...
   */
  public File getAttachmentFromBlobStorage(String fileName)
  ...
  private String createTempDirectory() throws BlobStorageClientException {
    try {
      ...
      Path tempDirectory =
          Files.createTempDirectory(workingDirectory.toPath(), "receipt-pdf-service");
      return tempDirectory.toAbsolutePath() + "/receiptPdf.pdf";
    } catch (IOException e) {
      logger.error(
          "Error creating the temp directory to download the PDF receipt from Blob Storage");
      throw new BlobStorageClientException(PDFS_600, PDFS_600.getErrorMessage(), e);
    }
  }

  private void downloadAttachment(String fileName, BlobClient blobClient, String filePath)
      throws BlobStorageClientException, AttachmentNotFoundException {
    try {
      blobClient.downloadToFileWithResponse(
          getBlobDownloadToFileOptions(filePath), Duration.ofSeconds(timeout), Context.NONE);
      ...
      if (e.getStatusCode() == 404) {
        errMsg =
            String.format(
                "PDF receipt with name: %s not found in Blob Storage: %s",
                sanitize(fileName), blobClient.getAccountName());
        logger.error(errMsg);
        throw new AttachmentNotFoundException(PDFS_602, errMsg, fileName, e);
      }
      errMsg =
          String.format(
              "Unable to download the PDF receipt with name: %s from Blob Storage: %s. Error"
                  + " message from server: %s",
              sanitize(fileName), blobClient.getAccountName(), e.getServiceMessage());
      logger.error(errMsg);
