...
public class ApiLoggingFilter {
    ...
    private String getParams(ContainerRequestContext ctx) {
        ...
        params.putAll(extractRequestParams(ctx.getUriInfo().getQueryParameters()));
        params.putAll(extractRequestParams(ctx.getUriInfo().getPathParameters()));

        return toJsonString(params);
    }

    private static Map<String, Object> extractRequestParams(MultivaluedMap<String, String> requestParams) {
        Map<String, Object> params = new HashMap<>();

        if (requestParams != null && !requestParams.isEmpty()) {
            for (var item : requestParams.entrySet()) {
                var key = item.getKey();
