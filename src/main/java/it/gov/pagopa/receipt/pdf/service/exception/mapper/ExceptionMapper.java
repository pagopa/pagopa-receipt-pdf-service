...
public class ExceptionMapper {
  ...
  public RestResponse<ErrorResponse> mapMissingFiscalCodeHeaderException(
      InvalidFiscalCodeHeaderException invalidFiscalCodeHeaderException) {
    Response.Status status = BAD_REQUEST;
    String message = "The provided fiscal code is invalid.";
    logger.error(message, invalidFiscalCodeHeaderException);
    return RestResponse.status(
        status,
        buildErrorResponse(invalidFiscalCodeHeaderException.getErrorCode(), status, message));
  }

  @ServerExceptionMapper
  public RestResponse<ErrorResponse> mapReceiptNotFoundException(
      ReceiptNotFoundException receiptNotFoundException) {
    Response.Status status = NOT_FOUND;
    String message =
        String.format(
            "Receipt with the provided third party id: %s not found",
            receiptNotFoundException.getReceiptId());
    logger.error(message, receiptNotFoundException);
    return RestResponse.status(
        status, buildErrorResponse(receiptNotFoundException.getErrorCode(), status, message));
  }

  @ServerExceptionMapper
  public RestResponse<ErrorResponse> mapAttachmentNotFoundException(
      AttachmentNotFoundException attachmentNotFoundException) {
    Response.Status status = NOT_FOUND;
    String message =
        String.format(
            "Attachment with the provided name: %s not found",
            attachmentNotFoundException.getAttachmentName());
    logger.error(message, attachmentNotFoundException);
    return RestResponse.status(
        status, buildErrorResponse(attachmentNotFoundException.getErrorCode(), status, message));
  }

  @ServerExceptionMapper
  public RestResponse<ErrorResponse> mapPdfServiceException(
      PdfServiceException pdfServiceException) {
    AppErrorCodeEnum errorCode = pdfServiceException.getErrorCode();
    String message =
        String.format(
            "Error during request elaboration, id: %s. Please contact support.", errorCode);
  ...
  public RestResponse<ErrorResponse> mapGenericException(Exception exception) {
    ...
    String message =
        String.format(
            "An unexpected error has occurred, id: %s. Please contact support.", errorCode);
    return getInternalServerErrorResponse(exception, errorCode, message);
  }

  private RestResponse<ErrorResponse> getInternalServerErrorResponse(
      Throwable t, AppErrorCodeEnum errorCode, String message) {
  ...
  private ErrorResponse buildErrorResponse(
      AppErrorCodeEnum errorCode, Response.Status status, String message) {
