...
public class AttachmentsServiceImpl implements AttachmentsService {
  ...
  /**
   * Retrieves the attachment details for a single receipt, validating the authorization of the
   * provided fiscal code to access the receipt.
   *
   ...
   * @throws FiscalCodeNotAuthorizedException if the fiscal code is not authorized to access the
   *     receipt
   */
  private AttachmentsDetailsResponse handleSingleReceiptAttachmentDetails(
    ...
    SearchTokenResponse searchTokenResponse =
        getSearchTokenResponse(thirdPartyId, requestFiscalCode);
   ...
   *
   * <p>If the {@code thirdPartyId} contains a business event ID (debtor case), it fetches the
   * corresponding cart payment and its attachment details. If not (payer case), it fetches the
   * payer's attachment details. The method validates the authorization of the provided fiscal code
   * to access the cart's attachments.
   *
  ...
  private AttachmentsDetailsResponse handleCartAttachmentDetails(
    ...
    if (isFiscalCodeNotAuthorized(token, bizEventId, cartForReceipt)) {
      String errMsg =
          String.format(
              "Fiscal code is not authorized to access the receipts for cart with id %s",
              sanitize(thirdPartyId));
      logger.error(errMsg);
      throw new FiscalCodeNotAuthorizedException(AppErrorCodeEnum.PDFS_700, errMsg);
    }
    ...
    if (bizEventId == null) {
      ...
      if (cartForReceipt.getPayload().getMdAttachPayer() == null) {
        String errMsg =
            String.format(
                "The retrieved receipt metadata for cart %s has null payer attachment info",
                sanitize(cartId));
        logger.error(errMsg);
        throw new InvalidReceiptException(AppErrorCodeEnum.PDFS_712, errMsg);
      }

      return buildCartAttachmentDetails(
          cartForReceipt,
          cartForReceipt.getPayload().getMdAttachPayer(),
          cartForReceipt.getPayload().getMessagePayer());
    }
    ...
    if (cartItem == null || cartItem.getMdAttach() == null) {
      String errMsg =
          String.format(
              "The retrieved receipt metadata for cart %s for the debtor with bizEventId: %s has null debtors attachment info",
    ...
    return buildCartAttachmentDetails(
        cartForReceipt, cartItem.getMdAttach(), cartItem.getMessageDebtor());
  }

  /**
   * Finds the debtor cart payment item in the given cart that matches the specified business event
   * ID.
   *
   * @param cartForReceipt the cart containing the list of payments
   * @param eventId the business event ID to match
   * @return the first matching {@link CartPayment} with non-null attachment and message for the
   *     debtor, or null if not found
   */
  private CartPayment findDebtorCartPaymentByBizEventId(
      CartForReceipt cartForReceipt, String eventId) {
  ...
  private void getSingleReceiptAttachment(
      String thirdPartyId, String requestFiscalCode, String attachmentUrl)
      throws ReceiptNotFoundException, InvalidReceiptException, FiscalCodeNotAuthorizedException {
    Receipt receiptDocument = getReceipt(thirdPartyId);
    SearchTokenResponse searchTokenResponse =
        getSearchTokenResponse(thirdPartyId, requestFiscalCode);
  ...
  private void getCartAttachment(
      String thirdPartyId, String requestFiscalCode, String attachmentUrl)
    ...
    boolean isFiscalCodeNotAuthorized =
        isFiscalCodeNotAuthorized(attachmentUrl, partial, searchTokenResponse, cartForReceipt);
   ...
   */
  private static boolean isFiscalCodeNotAuthorized(
    ...
    boolean isFiscalCodeNotAuthorized;
    if (partial.length > 1) {
      isFiscalCodeNotAuthorized =
          isDebtorFiscalCodeNotAuthorized(
              searchTokenResponse.getToken(), attachmentUrl, partial[1], cartForReceipt);
    } else {
      isFiscalCodeNotAuthorized =
          isPayerFiscalCodeNotAuthorized(
              searchTokenResponse.getToken(), attachmentUrl, cartForReceipt);
    }
  ...
  private Receipt getReceipt(String thirdPartyId)
    ...
    if (receiptDocument == null) {
      String errMsg =
          String.format("The retrieved receipt with id: %s, is null", sanitize(thirdPartyId));
      logger.error(errMsg);
    ...
    if (receiptDocument.getEventData() == null) {
      String errMsg =
          String.format(
              "The retrieved receipt with id: %s, has null event data", sanitize(thirdPartyId));
      logger.error(errMsg);
    ...
    if (receiptDocument.getMdAttach() == null
        && (!ANONIMO.equals(receiptDocument.getEventData().getDebtorFiscalCode()))) {
      String errMsg =
          String.format(
  ...
  private CartForReceipt getCartReceipt(String cartId)
      throws CartNotFoundException, InvalidCartException {
    ...
    if (allDebtorsAreNull) {
      String errMsg =
          String.format(
              "The retrieved cart with id: %s, has null debtors fiscal code", sanitize(cartId));
      logger.error(errMsg);
      throw new InvalidCartException(AppErrorCodeEnum.PDFS_709, errMsg);
    }

    var allDebtorsAttachAreNull =
        cartForReceipt.getPayload().getCart().stream()
            .filter(Objects::nonNull)
    ...
    if (allDebtorsAttachAreNull && cartForReceipt.getPayload().getMdAttachPayer() == null) {
      String errMsg =
          String.format(
              "The retrieved cart with id: %s, has null attachment info", sanitize(cartId));
      logger.error(errMsg);
    ...
    for (CartPayment elem : cartForReceipt.getPayload().getCart()) {
      if ((elem.getDebtorFiscalCode() != null
              ...
              && !elem.getDebtorFiscalCode()
                  .equals(cartForReceipt.getPayload().getPayerFiscalCode()))
          && elem.getMdAttach() == null) {
        String errMsg =
            String.format(
    ...
    if (cartForReceipt.getPayload().getPayerFiscalCode() != null
        && cartForReceipt.getPayload().getMdAttachPayer() == null) {
      String errMsg =
          String.format(
  ...
  private AttachmentsDetailsResponse buildCartAttachmentDetails(
      CartForReceipt cartReceiptDocument,
   ...
   */
  private static boolean isDebtorFiscalCodeNotAuthorized(
    ...
    if (matches.size() == 1) {
      ...
      isDebtorAuthorized =
          Objects.equals(requestFiscalCode, match.getDebtorFiscalCode())
              && mdAttach != null
   ...
   */
  private static boolean isPayerFiscalCodeNotAuthorized(
      String requestFiscalCode, String attachmentUrl, CartForReceipt cartForReceipt) {
    boolean isPayerAuthorized =
        cartForReceipt.getPayload().getMdAttachPayer() != null
  ...
  /**
   * Checks if the given fiscal code is not authorized to access the cart's attachments details.
   *
   * <p>If {@code eventId} is {@code null}, the method checks if the fiscal code matches the payer's
   * fiscal code and if the payer's attachment metadata is present. Otherwise, it checks if the
   * fiscal code matches any debtor's fiscal code and business event ID in the cart, and if the
   * debtor's attachment metadata is present.
   *
   * @param requestFiscalCode the fiscal code to check authorization for
   * @param eventId the business event ID to match for debtor authorization, or {@code null} for
   *     payer
   * @param cartForReceipt the receipt cart object
   * @return {@code true} if the fiscal code is not authorized, {@code false} otherwise
   */
  private boolean isFiscalCodeNotAuthorized(
    ...
    if (eventId == null) {
      ...
      boolean isDebtorAuthorized =
          cartForReceipt.getPayload().getCart() != null
              && cartForReceipt.getPayload().getCart().stream()
                  .filter(elem -> elem != null && elem.getMdAttach() != null)
                  .anyMatch(
                      elem ->
