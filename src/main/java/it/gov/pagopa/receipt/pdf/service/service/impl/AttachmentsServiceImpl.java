...
public class AttachmentsServiceImpl implements AttachmentsService {
  ...
  private AttachmentsDetailsResponse handleCartAttachmentDetails(
    ...
    if (cartItem == null || cartItem.getMdAttach() == null) {
      String errMsg =
          String.format(
              "The retrieved receipt metadata for cart %s for the debtor with bizEventId: %s has"
                  + " null debtors attachment info",
  ...
  private void getSingleReceiptAttachment(
    ...
    if (isFiscalCodeNotAuthorized(searchTokenResponse.getToken(), attachmentUrl, receiptDocument)) {
      String errMsg =
          String.format(
              "Fiscal code is not authorized to access the receipts with name: %s, for receipt with"
                  + " id %s",
  ...
  private void getCartAttachment(
    ...
    if (isFiscalCodeNotAuthorized) {
      String errMsg =
          String.format(
              "Fiscal code is not authorized to access the receipts with name: %s, for cart with id"
                  + " %s",
              sanitize(attachmentUrl), sanitize(thirdPartyId));
      logger.error(errMsg);
      throw new FiscalCodeNotAuthorizedException(AppErrorCodeEnum.PDFS_706, errMsg);
  ...
  private Receipt getReceipt(String thirdPartyId)
    ...
    if (!isReceiptUnique(receiptDocument) && receiptDocument.getMdAttachPayer() == null) {
      String errMsg =
          String.format(
              "The retrieved receipt with id: %s, has different debtor and payer fiscal codes but"
                  + " has null attachment info for payer",
              sanitize(thirdPartyId));
      logger.error(errMsg);
