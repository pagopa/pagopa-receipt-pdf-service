...
public class ReceiptCosmosClientImpl implements ReceiptCosmosClient {
  ...
  @ReceiptsContainer private final CosmosContainer containerReceipts;
  ...
  public Receipt getReceiptDocument(String thirdPartyId) throws ReceiptNotFoundException {
    ...
    CosmosPagedIterable<Receipt> queryResponse =
        containerReceipts.queryItems(query, new CosmosQueryRequestOptions(), Receipt.class);

    if (!queryResponse.iterator().hasNext()) {
      String errMsg =
          String.format(
              "Receipt with id %s not found in the defined container: %s",
