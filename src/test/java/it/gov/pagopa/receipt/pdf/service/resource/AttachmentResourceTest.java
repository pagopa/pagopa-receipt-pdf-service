...
class AttachmentResourceTest {
  ...
  @Inject private AttachmentResource sut;
  ...
  void getAttachmentDetailsSuccess() {
    ...
    AttachmentsDetailsResponse attachment =
        AttachmentsDetailsResponse.builder()
            .attachments(
    ...
    doReturn(attachment)
        .when(attachmentsServiceMock)
        .getAttachmentsDetails(THIRD_PARTY_ID, FISCAL_CODE);

    String responseString =
        given()
            .queryParam("fiscal_code", FISCAL_CODE)
            .when()
            .get("/messages/" + THIRD_PARTY_ID)
            .then()
            .statusCode(200)
    ...
    AttachmentsDetailsResponse response =
        objectMapper.readValue(responseString, AttachmentsDetailsResponse.class);
  ...
  void getAttachmentDetailsFailGetReceiptError400() {
    doThrow(new ReceiptNotFoundException(PDFS_800, ""))
        .when(attachmentsServiceMock)
        .getAttachmentsDetails(THIRD_PARTY_ID, FISCAL_CODE);
  ...
  void getAttachmentDetailsFailGetReceiptError500() {
    doThrow(new FiscalCodeNotAuthorizedException(PDFS_700, ""))
        .when(attachmentsServiceMock)
        .getAttachmentsDetails(THIRD_PARTY_ID, FISCAL_CODE);
  ...
  void getAttachmentSuccess() {
    ...
    doReturn(file)
        .when(attachmentsServiceMock)
        .getAttachment(THIRD_PARTY_ID, FISCAL_CODE, ATTACHMENT_URL);

    byte[] response =
        given()
            .queryParam("fiscal_code", FISCAL_CODE)
            .when()
            .get(String.format("/messages/%s/%s", THIRD_PARTY_ID, ATTACHMENT_URL))
            .then()
            .statusCode(200)
            .contentType("application/pdf")
  ...
  void getAttachmentFailGetReceiptError400() {
    doThrow(new AttachmentNotFoundException(PDFS_602, ""))
        .when(attachmentsServiceMock)
        .getAttachment(THIRD_PARTY_ID, FISCAL_CODE, ATTACHMENT_URL);
  ...
  void getAttachmentFailGetReceiptError500() {
    doThrow(new FiscalCodeNotAuthorizedException(PDFS_706, ""))
        .when(attachmentsServiceMock)
        .getAttachment(THIRD_PARTY_ID, FISCAL_CODE, ATTACHMENT_URL);
