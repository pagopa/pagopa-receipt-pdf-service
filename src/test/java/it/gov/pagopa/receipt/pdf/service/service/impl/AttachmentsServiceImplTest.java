...
class AttachmentsServiceImplTest {
    ...
    @BeforeEach
    void init() {
        Mockito.reset(receiptCosmosServiceMock, receiptBlobClientMock, restClientMock);
    ...
    void getAttachmentCartPayerSuccess() {
        ...
        doReturn(cart).when(cartReceiptCosmosServiceMock).getCartReceipt(anyString());
        ...
        InputStream result = sut.getAttachment("test_CART_", payerFiscalCode, fileNamePayer);
    ...
    void getAttachmentCartDebtorSuccess() {
        ...
        InputStream result = sut.getAttachment("test_CART_biz1", debtorFiscalCode, fileNameDebtor);
    ...
    void getAttachmentsDetails_receipt_success_debtor() throws Exception {
        ...
        when(receiptCosmosServiceMock.getReceipt(anyString())).thenReturn(receipt);
    ...
    void getAttachmentsDetails_cart_success_payer() throws Exception {
        ...
        when(cartReceiptCosmosServiceMock.getCartReceipt(anyString())).thenReturn(cart);
        when(restClientMock.searchToken(any())).thenReturn(new SearchTokenResponse(payer));
