...
class ReceiptBlobClientImplTest {
  ...
  @Inject private ReceiptBlobClient sut;

  @Inject private BlobContainerClient blobContainerClientMock;
  ...
  void getAttachmentFromBlobStorageSuccess() {
    doReturn(null)
        .when(blobClientMockMock)
        .downloadToFileWithResponse(
            any(BlobDownloadToFileOptions.class), any(Duration.class), any(Context.class));
  ...
  void getAttachmentFromBlobStorageFailDownloadThrowsUncheckedIOException() {
    ...
    BlobStorageClientException e =
        assertThrows(
            BlobStorageClientException.class, () -> sut.getAttachmentFromBlobStorage(anyString()));
  ...
  void getAttachmentFromBlobStorageFailDownloadAttachmentNotFound() {
    ...
    doThrow(new BlobStorageException("", responseMock, null))
        .when(blobClientMockMock)
        .downloadToFileWithResponse(
            any(BlobDownloadToFileOptions.class), any(Duration.class), any(Context.class));
    ...
    AttachmentNotFoundException e =
        assertThrows(
            AttachmentNotFoundException.class, () -> sut.getAttachmentFromBlobStorage(anyString()));
